...
class HomeScreen extends Component {
  ...
  componentDidMount() {
    if (Platform.OS === 'android') {
      // Android
      NativeModules.ReactBridge.add(1, 2).then(res => this.setState({ result: res }));
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <Text>{this.state.result === null ? 'Loading…' : this.state.result}</Text>
        <Text>
          {this.state.peripheralState === null
            ? 'Will show peripheral state here if using as peripheral'
            : this.state.peripheralState}
        </Text>
        <FlatList
